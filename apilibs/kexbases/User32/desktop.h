...
#if 0
static WORD inline Enable(LPVOID lpDevInfo, WORD wAction, LPSTR lpDestDevType, LPSTR lpOutputFile, LPVOID lpData)
{
	typedef WORD (WINAPI *ENABLE)(LPVOID,WORD,LPSTR,LPSTR,LPVOID);
	HINSTANCE hModule;
	ENABLE _Enable;
	WORD Result;

	hModule = (HINSTANCE)LoadLibrary16("VGA.DRV");

	if(hModule == NULL)
		return 0;

	_Enable = (ENABLE)GetProcAddress16(hModule, "ENABLE");
	...
	__asm push [lpDevInfo]
	__asm push [wAction]
	__asm push [lpDestDevType]
	__asm push [lpOutputFile]
	__asm push [lpData]
	__asm mov edx, [_Enable]
	__asm call QT_Thunk
	__asm mov [Result], ax
...
static void inline Disable(LPPDEVICE lpDestDev)
{
	typedef void (WINAPI *DISABLE)(LPPDEVICE);
	HINSTANCE hModule;
	DISABLE _Disable;
	...
	_Disable = (DISABLE)GetProcAddress16(hModule, "DISABLE");
	...
	__asm push [lpDestDev]
	__asm mov edx, [_Disable]
	__asm call QT_Thunk
